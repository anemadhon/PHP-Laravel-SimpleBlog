<x-main-layout>
    <x-slot name="header">
        <p class="flex items-center">
            ...
            <span>{{ __('List of Articles - Category:') }}<span class="underline">{{ __($category->name) }}</span></span>
        </p>
    </x-slot>
    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        ...
        <div class="flex justify-center mb-6 md:justify-end">
            ...
            >
                ...
                <form action="{{ route('main.articles.category', ['category' => $category]) }}" method="get">
                    <input
        ...
        <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4 flex justify-center">
            @forelse ($category->articles as $article)
                <div class="max-w-md py-4 px-8 bg-white shadow-lg rounded-lg my-12">
                    ...
                    <div class="mt-4 -mx-4">
                        <span class="mt-3 text-xs text-indigo-400">{{ __('Category: ') }}</span><a class="mt-3 text-xs text-indigo-400" href="{{ route('main.articles.category', ['category' => $category]) }}">{{ ucwords($category->name) }}</a>
                    </div>
                    <div class="mt-2 -mx-4">
